class Admin::ProductsController < Admin::BaseController
  ...
  def index
    @products = Product.includes(:category, :prices).order(:name)
  end
  ...
  def destroy
    ...
    begin
      ...
      if result
        ...
        puts "Deleted product ID: #{product_id}, Name: #{product_name}"
...
def update_price_if_changed
  return unless params[:price].present?

  current_price = @product.prices.where(end_date: nil).last
